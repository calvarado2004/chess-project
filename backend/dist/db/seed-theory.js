...
async function seedTheoryLessons() {
    ...
    for (const file of files) {
        ...
        const { id, title, category, difficulty, description, estimatedMinutes, keyConcepts, } = lesson;
        ...
        try {
            await query(`INSERT INTO theory_lessons (id, title, category, difficulty, description, estimated_minutes, key_concepts, content)
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           content = EXCLUDED.content`, [id, title, category, difficulty, description, estimatedMinutes ?? 15, keyConceptsJson, content]);
            console.log(`  [OK] ${id}`);
        }
        catch (err) {
...
seedTheoryLessons();
//# sourceMappingURL=seed-theory.js.map
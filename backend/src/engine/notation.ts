...
// ===================== FEN Generation =====================
export function generateFEN(board: number[][], turn: 'w' | 'b', castlingRights: { wK: boolean; wQ: boolean; bK: boolean; bQ: boolean }, enPassantTarget: Coord | null, halfmoveClock: number, fullmoveNumber: number): string {
  let fen = '';
  for (let r = 0; r < 8; r++) {
    let empty = 0;
    for (let c = 0; c < 8; c++) {

...
router.get('/lessons', async (_req, res) => {
  try {
    const result = await query(
      `SELECT id, title, category, difficulty, estimated_minutes AS estimatedMinutes,
              key_concepts
       FROM theory_lessons
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    ...
    try {
      ...
      const result = await query(
        `INSERT INTO theory_progress (user_id, lesson_id, completed, last_position_index, exercise_score, completed_at)
         VALUES ($1, $2, $3, $4, $5, $6)
         ...
         DO UPDATE SET
           ...
           updated_at = now()
         RETURNING id, user_id, lesson_id, completed, last_position_index, exercise_score, completed_at, updated_at`,
        [req.userId, lessonId, completed ?? false, lastPositionIndex ?? 0, exerciseScore ?? 0, completedAt]
      );
...
router.get('/progress', authenticate, async (req: AuthRequest, res) => {
  ...
  try {
    const result = await query(
      `SELECT tp.id, tp.lesson_id AS lessonId, tp.completed,
